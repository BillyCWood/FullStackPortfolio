...
<template>
    <div class="grid grid-cols-1 grid-rows-4 lg:grid-cols-2 lg:grid-rows-2 min-h-screen pt-32 text-lg lg:text-xl font-light lg:gap-y-10 max-xl:px-5">
        <div id="exp-list" class="">
            <h1 class="text-primary-light text-4xl lg:text-6xl font-medium">Experience</h1>
            <button class="w-56 py-4 font-normal lg:hidden flex justify-center items-center gap-x-2 group -translate-x-3">
            ...
            <ul class="flex max-lg:gap-x-7 lg:flex-col lg:gap-y-8 my-10 lg:border-l-2 border-secondary-light max-lg:overflow-scroll">
                <li v-for="(job, index) in jobs" :key="index" class="hover:cursor-pointer p-3 lg:max-w-80" :class="styledJob(index)" @click="selectedJob = index">{{ job.name }}</li>
            </ul>
            <a :href="jobs[selectedJob].url" target="_blank" class="hidden lg:flex">
                <Button message="View Sites" />
            </a>
        </div>

        <div id="exp-description" class="flex flex-col gap-y-4 max-lg:-translate-y-32">
            ...
            <a :href="jobs[selectedJob].url" target="_blank" class="lg:hidden">
                <Button message="View Sites" />
            </a>
        </div>


        <div id="proj-list" class="">
            <h1 class="text-primary-light text-4xl lg:text-6xl font-medium">Projects</h1>
            <ul class="flex max-lg:gap-x-7 lg:flex-col lg:gap-y-8 my-10 lg:border-l-2 border-secondary-light max-lg:overflow-scroll">
                <li v-for="(project, index) in projects" :key="index" class="hover:cursor-pointer p-3 lg:max-w-60" :class="styledProj(index)" @click="selectedProj = index">{{ project.name }}</li>
            </ul>
            <a :href="projects[selectedProj].url" target="_blank" class="hidden lg:flex">
                <Button message="View Projects" />
            </a>
        </div>

        <div id="proj-description" class="flex flex-col gap-y-4 pt-28 max-lg:-translate-y-96">
            ...
            <a :href="projects[selectedProj].url" target="_blank" class="lg:hidden">
                <Button message="View Projects" />
            </a>
        </div>
    </div>
